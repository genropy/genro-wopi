...
Example:
    Creating and managing sessions::
        ...
        from genro_wopi import WopiProxy

        wopi = WopiProxy()
        await wopi.init()

        sessions = wopi.db.table("wopi_sessions")
...
from genro_proxy.sql import String, Table, Timestamp
...
class WopiSessionsTable(Table):
    ...
    async def get(self, session_id: str) -> dict[str, Any] | None:
        ...
        """
        return await self.record(where={"id": session_id}, ignore_missing=True) or None
    ...
    async def get_by_token(self, access_token: str) -> dict[str, Any] | None:
        ...
        Returns:
            Session dict or None if not found.
        """
        return await self.record(where={"access_token": access_token}, ignore_missing=True) or None
    ...
    async def get_by_file_id(self, file_id: str) -> dict[str, Any] | None:
        ...
        Returns:
            Session dict or None if not found.
        """
        return await self.record(where={"file_id": file_id}, ignore_missing=True) or None
    ...
    async def cleanup_expired(self) -> int:
        ...
        result = await self.db.execute(
            f"DELETE FROM {self.name} WHERE expires_at <= :expires_at",
            {"expires_at": now},
        )
        return result
